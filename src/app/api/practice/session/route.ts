...
const MODES: StudyMode[] = ["review", "cram", "quiz"];

/** GET /api/practice/session?mode=review|cram|quiz[&unit=ID|&class=ID] */
export const GET = handler(async (request: Request) => {
  const url = new URL(request.url);
  const mode = url.searchParams.get("mode") as StudyMode;
  if (!MODES.includes(mode)) throw badRequest("mode must be review, cram or quiz.");
