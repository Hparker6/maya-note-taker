...
- **Automatic backups:** the app copies its database (notes, handwriting, study sheets, flashcards, quizzes) to `data/backups` shortly after starting, then about once a day, keeping the last 14, plus a copy before any update. To go back to one, run `npm run restore`: it lists the backups with their dates, closes the app, keeps a copy of what you have now (so a restore can be undone too), and puts the chosen one back. PDFs stay in `data/files` and aren't affected.
